...
def handle_api_key_configuration(server_args: Any) -> None:
    """Resolve mutually exclusive inline and file-backed API keys.

    :param server_args: Raw server argument record under resolution.
    """

    cfg = resolving_view(server_args)
    if cfg.api_key is not None and cfg.api_key_file is not None:
        raise ValueError("--api-key and --api-key-file cannot be combined.")
    if cfg.api_key_file is None:
        return
    declare_resolution(
        server_args,
        "handle_api_key_configuration",
        api_key=_read_api_key_file(cfg.api_key_file),
    )
