...
class PackedPrefillSubmission:
    ...
    :ivar auxiliary_source_index: Source metadata row copied by canonical TP0.
...
class PackedPrefillRuntime:
    ...
    def _prepare_submission(
        ...
        if route.topology.destination_tp_size != 1:
            raise ValueError("packed source actor requires TP1 decode")
        chunk_key = PackedChunkKey(
    ...
    def _register_prepare(self, record: _PrefillRequestRecord) -> PackedPrepare:
        ...
        spec, source_binding = build_prefill_chunk(
            ...
            source_tp_size=self._manifest.source_tp_size,
            destination_tp_size=submission.destination.route.topology.destination_tp_size,
            destination_tp_rank=submission.destination.route.topology.destination_tp_rank,
            writers=self._manifest.writers,
        )
    ...
    def prepare_transaction(
        ...
        *,
        ...
        spec = build_decode_spec(
            ...
            spans=tuple(spans),
            kv_args=self._manager.kv_args,
            expected_writers=snapshot.writer_manifest.writers,
