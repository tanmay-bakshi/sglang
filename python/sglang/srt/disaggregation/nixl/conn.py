...
class NixlKVManager(CommonKVManager):
    ...
    def _packed_source_route(
        ...
        """Resolve one room's exact packed TP1 destination.
        ...
        if len(packed_infos) != 1:
            raise RuntimeError(
                "packed asymmetric transfer requires one TP1 destination"
            )
        ...
        if registration.decode_tp_size != 1 or registration.decode_tp_rank != 0:
            raise RuntimeError("packed asymmetric transfer requires TP1 decode")
        if (
    ...
    def _packed_source_components(
        ...
        :param source_main_pages: Complete request-local source KV pages.
    ...
    def _add_remote_peer(self, decode_kv_args: KVArgsRegisterInfo):
        ...
        if uses_packed_runtime:
            ...
            if decode_kv_args.decode_tp_size != 1 or decode_kv_args.decode_tp_rank != 0:
                raise RuntimeError(
                    "packed decoder registration must describe TP1 rank zero"
                )
    ...
    def send_metadata(
        ...
        *,
        ...
        canonical_aux_sources = [
            ...
            if peer.attn_tp_rank == 0 and peer.attn_cp_rank == 0 and peer.pp_rank == 0
        ]
